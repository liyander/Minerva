...
export async function initializeDatabaseLegacyIfNeeded() {
  ...
  try {
    ...
    if (tableCheck.length > 0) {
      ...
      await addColumnIfMissing('community_messages', 'attachment_name', 'VARCHAR(255) NULL')
      await addColumnIfMissing('community_messages', 'attachment_type', 'VARCHAR(120) NULL')
      await addColumnIfMissing('community_messages', 'attachment_size', 'INT DEFAULT 0')
      await addColumnIfMissing('community_messages', 'attachment_data', 'LONGTEXT NULL')
